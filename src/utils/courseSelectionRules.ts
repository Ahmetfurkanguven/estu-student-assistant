...
import { StudentRecord, Course, ScheduleOffering, GPAResult } from '../types';
import { ALL_COURSES } from '../data/courses';

/**
 * Rules based on: ESKİŞEHİR TEKNİK ÜNİVERSİTESİ ÖN LİSANS VE LİSANS EĞİTİM-ÖĞRETİM VE SINAV YÖNETMELİĞİ
 * Published: 9 Eylül 2025
 */

export interface ProposedCourse {
    course: Course;
    reason: string;
    priority: number; // 1: Critical (Must), 2: Probation (Conditional), 3: Standard (Flow)
    isRepeat: boolean;
}
 ...
 */
function checkPrereqs(course: Course, passedCourses: Set<string>): boolean {
...
export function generateCourseProposal(
    ...
    gpa: GPAResult
): { proposal: ProposedCourse[]; totalECTS: number; logs: string[] } {
    const logs: string[] = [];
    ...
    for (const course of coursePool) {
        ...
        if (passedCourses.has(course.code)) continue;

        // Skip if already proposed (as retake)
        if (proposal.find(p => p.course.code === course.code)) continue;
        ...
        if (!isOffered(course.code, offerings)) continue;
    ...
    const finalProposal: ProposedCourse[] = [];
    let currentECTS = 0;
    const MAX_ECTS = 45;
