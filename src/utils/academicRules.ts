 ...
 */
export function applyIntibak(records: StudentRecord[]): StudentRecord[] {
 ...
 */
export function checkPrerequisites(courseCode: string, completedCourses: Set<string>): { canTake: boolean; missing: string[] } {
 ...
 */
export function checkEEM413Eligibility(records: StudentRecord[], gpa: GPAResult): { eligible: boolean; reasons: string[] } {
 ...
 */
export function checkSpecialization(records: StudentRecord[], areaId: string): { meets: boolean; progress: string } {
    const area = SPECIALIZATION_AREAS.find(a => a.id === areaId);
    if (!area) return { meets: false, progress: '0/5 ders' };
    const takenMS = records.filter(r =>
        r.grade.passed &&
