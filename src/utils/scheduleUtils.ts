import type { ScheduleOffering, ParsedSchedule } from '../types';
...
// --- Conflict Detection ---
export function detectScheduleConflicts(selected: ScheduleOffering[]): Array<{ courses: string[]; time: string }> {
    const conflicts: Array<{ courses: string[]; time: string }> = [];
    for (let i = 0; i < selected.length; i++) {
        for (let j = i + 1; j < selected.length; j++) {
            const a = selected[i];
            const b = selected[j];
            if (a.async || b.async || a.day !== b.day) continue;
...
export function parseScheduleFromItems(items: PdfTextItem[]): ScheduleOffering[] {
    ...
    pages.forEach((pageItems, pageNum) => {
        ...
        const timeSlots: { y: number, start: number, end: number }[] = [];
        ...
        const dayStarts: { y: number, dayIndex: number }[] = [];
        let dayCounter = 0;
        ...
        for (const item of pageItems) {
            ...
            if (courseCode.length < 5) continue;
            if (['SAAT', 'DERS', 'SINIF'].some(s => courseCode.includes(s))) continue;
            ...
            if (closestTime.start === 0) continue;
...
function mergeConsecutiveSlots(offerings: ScheduleOffering[]): ScheduleOffering[] {
...
// Legacy exports
export function parseScheduleText(text: string): ScheduleOffering[] {
    return [];
}

export function parseEstuScheduleJson(items: PdfTextItem[]): ParsedSchedule {
    return {
...
export function convertJsonToSchedule(parsed: ParsedSchedule): ScheduleOffering[] {
    return [];
}
