 ...
 */
function extractCourseCode(tokenRaw) {
    if (!tokenRaw)
        return "";
...
export function parseTranscriptText(text) {
        ...
        if (match && currentSemester) {
            ...
            if (isCourseCode(potentialCode)) {
                ...
                if (gradeIndex !== -1) {
                    ...
                    let equivalentCourse = undefined;
                    for (const rt of restTokens) {
    ...
    const uniqueRecords = records.filter((r, index, self) => index === self.findIndex((t) => (t.semester === r.semester &&
