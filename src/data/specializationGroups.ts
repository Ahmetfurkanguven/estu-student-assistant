...
export interface SpecializationCourse {
    code: string;
    name: string;
    isMandatory: boolean; // (Z) ile işaretli olanlar
    prerequisite: string | null; // Önşart ders kodu
    term: 'Güz' | 'Bahar';
}

export interface SpecializationGroupDefinition {
    id: string;
    name: string; // Örn: Elektronik / Electronics
    courses: SpecializationCourse[];
}

export const SPECIALIZATION_GROUPS: SpecializationGroupDefinition[] = [
    {
        id: 'electronics',
